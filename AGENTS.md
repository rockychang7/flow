...
**正文标题一律从 `##` 起,不要用 `#`。** 页面上唯一的 h1 是 frontmatter 的 `title`(由 ArticleHeader 渲染,24px);正文 `#` 同为 24px,会与文题打平,且让一个页面出现多个 h1。

正文字阶:`##` 20px / `###` 18px / `####` 16px。目录(TableOfContents)收 h2~h4 三级,更深的层级不进目录。
